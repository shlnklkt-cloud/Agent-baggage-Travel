# Travel Insurance Claims Portal - User Credentials

## All Test Users

### Original Users

#### 1. Rachel Ng
- **Passport/NRIC:** `CSGHY654JK`
- **Policy Number:** `TRV-2026-001487`
- **Policy Type:** Income Travel Insurance - Premier Plan
- **Status:** ACTIVE

#### 2. Broker Account
- **Passport/NRIC:** `CSGHY456JK`
- **Policy Number:** `TRV-2026-001687`
- **Policy Type:** Income Travel Insurance - Business Plan
- **Status:** ACTIVE

---

### New Users (Added)

#### 3. Mei Ling Chen
- **Passport/NRIC:** `CSGHY623JK`
- **Policy Number:** `TRV-2024-79045`
- **Policy Type:** Income Travel Insurance - Premier Plan
- **Status:** ACTIVE
- **Travel Itinerary:** SIN → BKK (SQ636), BKK → SIN (SQ637)

#### 4. Cheryl Chan
- **Passport/NRIC:** `CSGHY622JK`
- **Policy Number:** `TRV-2024-79145`
- **Policy Type:** Income Travel Insurance - Premier Plan
- **Status:** ACTIVE
- **Travel Itinerary:** SIN → SYD (SQ178), SYD → SIN (SQ179)

#### 5. Elizabeth Choy
- **Passport/NRIC:** `CSGHY664JK`
- **Policy Number:** `TRV-2024-79245`
- **Policy Type:** Income Travel Insurance - Premier Plan
- **Status:** ACTIVE
- **Travel Itinerary:** SIN → HKG (SQ254), HKG → SIN (SQ255)

#### 6. Kelly Pan
- **Passport/NRIC:** `CSGHY224JK`
- **Policy Number:** `TRV-2024-78946`
- **Policy Type:** Income Travel Insurance - Premier Plan
- **Status:** ACTIVE
- **Travel Itinerary:** SIN → CDG (SQ231), CDG → SIN (SQ232)

#### 7. Sophia Poh
- **Passport/NRIC:** `CSGHY304JK`
- **Policy Number:** `TRV-2024-79747`
- **Policy Type:** Income Travel Insurance - Premier Plan
- **Status:** ACTIVE
- **Travel Itinerary:** SIN → NRT (SQ828), NRT → SIN (SQ829)

#### 8. Emily Wong
- **Passport/NRIC:** `CSBNY384JK`
- **Policy Number:** `TRV-2024-79048`
- **Policy Type:** Income Travel Insurance - Premier Plan
- **Status:** ACTIVE
- **Travel Itinerary:** SIN → LHR (SQ322), LHR → SIN (SQ321)

---

## How to Login

1. Go to the login page
2. Enter your **Passport/NRIC/FIN Number** (e.g., `CSGHY623JK`)
3. Enter your **Policy Number** (e.g., `INC-TRV-2024-79045`)
4. Click **Login**

## Features Available After Login

- File baggage delay claims
- File baggage loss/damage claims
- Lost travel documents claims
- Medical emergency claims
- Trip postponement claims
- Trip cancellation claims
- Trip shortening claims
- Trip disruption claims
- View travel itinerary
- Track claim status

---

**Note:** All users have active travel insurance policies and can access all claim features.
